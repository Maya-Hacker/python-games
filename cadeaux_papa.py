...
GOAL = 4 # The exact number of water to have in a bucket to win.
steps = 0 # Keep track of hom many steps the player needed to solve this.

# The amount of water in each bucket:
waterInBucket = {'8': 0, '5': 0, '3':0}

while True: # Main game loop.
    # Dispaly the current state of the buckets:
    print()
    print('Try to get ' + str(GOAL) + 'L of water into one of these buckets:')

    waterDisplay = [] # Contains strings for water or emopty space.
    ...
    for i in range(1, 9):
        if waterInBucket['8'] < i:
            ...
            waterDisplay.append('WWWWW') # Add water.
...
'''.format(*waterDisplay))
    ...
    # Check if any buckets have the goal amount of water:
    ...
    while True: # Keep asking valid bucket is entered.
        print('Select bucket 8, 5, 3 or QUIT:')
    ...
    if move == 'F':
        ...
        # Let the player select a bucket to pur into:
        while True: # Keep asking until valid bucket is entered.
            print('Slect a bucket to pour into: 8, 5 or 3')
            dstBucket = input('> ').upper()
            if dstBucket in ('8', '5', '3'):
                break # Player has selecter a valid bucket.
