...
STREAM_CHARS = ['0', '1', '5', '*']  # (!) Try changing this to other characters.
...
# We can't print to the last column on Windows without it adding a newline automaticaly, so reduce the width by one:
WIDTH -= 1
...
try:
    # For each column, when the counter is 0, no stream in shown. Otherwise, it acts as a counter for how many times a 1 or a 0 should be displayed in that column.
    columns = [0] * WIDTH
    while True:
        ...
        sys.stdout.flush()  # Make sure text appears on screen.
        time.sleep(PAUSE)
