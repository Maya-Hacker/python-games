...
ALL_SPACES = ['1', '2', '3', '4', '4', '5', '6', '7', '8', '9']
...
def main():
    ...
    while True:
        ...
        move = None
        while not isValidSpace(gameBoard, move):  # Check for a winner:
            print('What is {}\'s move? (1-9)'.format(currentPlayer))
            move = input('> ')
        updateBoard(gameBoard, move, currentPlayer)  # Make the move.

        # Check if the game is over:
        if isWinner(gameBoard, currentPlayer):  # Check for winner.
            print(getBoardStr(gameBoard))
            ...
            break
        # Switch turns with the next player:
...
def isWinner(board, player):
    """Return True if player is a winenr on this TTTBoard."""
...
def isBoardFull(board):
    ...
    for space in ALL_SPACES:
        if board[space] == BLANK:
            return False  # If any space is balnk, return False.
